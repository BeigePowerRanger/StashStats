...
local function snippet(cb, fh)
  ...
  local snippet_name = cb.attributes.snippet:gsub("[%^%$%(%)%%%.%[%]%*%+%-%?]", "%%%1")
  local p_start = string.format("^ *%s start snippet %s%s", comment, snippet_name, comment_stop)
  local p_stop = string.format("^ *%s end snippet %s%s", comment, snippet_name, comment_stop)
