...
In Python client Pydantic models (`StashItem`), the following normalization occurs automatically on ingestion:
1. If `packs` is empty but `primary_pack` is present, `packs` is populated as `[primary_pack]`.
2. If `skeins`, `total_yards`, `total_grams`, or `total_meters` are None on the top-level object, values are populated from `primary_pack`.
3. If `colorway_name` or `dye_lot` are empty on the top-level object, values fallback to `primary_pack.colorway` and `primary_pack.dye_lot`.
