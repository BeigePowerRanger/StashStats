            ...
            [
                dbc.Col(
                    dbc.Button(
                        fname,
                        id={
                            "type": "project-pdf-view-btn",
                            "index": str(project_id),
                            "filename": fname,
                        },
                        color="link",
                        size="sm",
...
def paginate_projects(
    projects: list[dict[str, Any]], page: int, page_size: int = 10
...
def create_project_accordion_item(
    project: dict[str, Any], user_id: str | int = "default"
) -> dbc.AccordionItem:
    ...
    if status_name:
        ...
        badges.append(dbc.Badge(status_name, color=status_color, className="ms-2"))

    progress = project.get("progress")
    if progress is not None:
        badges.append(dbc.Badge(f"{progress}%", color="info", className="ms-1"))
    ...
    metadata = html.Div(
        [
            ...
            html.Span(
                f"Finished: {completed}" if completed else "", className="me-3 text-muted small"
            ),
        ],
        className="mb-3",
    )

    progress_bar = dbc.Progress(value=progress or 0, color="info", striped=True, className="mb-3")

    existing_pdfs = project.get("existing_pdfs")
    if existing_pdfs is None:
        user_id = project.get("user_id", user_id)
        from stashstats.storage import DEFAULT_DATA_DIR, list_project_pdfs
    ...
    pdf_section = html.Div(
        [
            ...
            html.Div(
                id={"type": "project-pdf-error", "index": str(project_id)},
                className="text-danger small mt-1",
            ),
            html.Div(
                create_pdf_file_list(existing_pdfs, project_id, user_id),
                id={"type": "project-pdf-list", "index": str(project_id)},
            ),
...
def create_grouped_projects_accordion(
    projects: list[dict[str, Any]], user_id: str | int = "default"
) -> Any:
